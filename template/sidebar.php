...
// Fetch user's full name and email from database
...
/* ============================================================
   5. SIDEBAR HEADER  (topstrip + profile)
============================================================ */
.sidebar-header {
    flex-shrink: 0;
}

/* ── top strip: logo left | icons right ── */
.sidebar-topstrip {
    display: flex;
    align-items: center;
...
.sidebar-logo img {
    ...
    object-fit: contain;
    display: block;
    filter: brightness(1.15) drop-shadow(0 1px 3px rgba(0, 0, 0, 0.35));
}
...
.sidebar-profile-dropdown {
    ...
    box-shadow: 0 8px 24px rgba(15, 36, 108, 0.16);
    z-index: 2000;
    opacity: 0;
    visibility: hidden;
...
.sidebar-dropdown-signout {
    color: #dc2626;
}

.sidebar-dropdown-signout i {
    color: #dc2626;
}

.sidebar-dropdown-signout:hover {
    background: #fff5f5;
    color: #dc2626;
}
...
.zoom-modal {
    ...
    width: 90%;
    max-width: 320px;
    box-shadow: 0 20px 50px rgba(15, 36, 108, 0.2);
    animation: zoomModalIn 0.22s cubic-bezier(0.34, 1.56, 0.64, 1);
}
...
.zoom-btn:hover { background: var(--blue-600); color: #fff; border-color: var(--blue-600); }
.zoom-btn:disabled { opacity: 0.35; cursor: not-allowed; }
.zoom-btn:disabled:hover { background: var(--bg); color: var(--text-900); border-color: var(--border); }
...
.zoom-reset:hover { background: #EEF2FF; border-color: var(--blue-600); color: var(--blue-600); }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-header">
            ...
            <!-- top strip: logo left | icons right -->
            <div class="sidebar-topstrip">
                <div class="sidebar-logo">
                    <img src="images/siems.png" alt="SIEMS Logo">
                </div>
                <div class="sidebar-header-icons">
                    <button class="header-icon-btn" id="sidebarNotifBtn" title="Notifications">
                        <i class='bx bx-bell'></i>
                        <span class="badge-dot"></span>
                    </button>
                    <div class="sidebar-profile-menu" style="position:relative;">
                        <button class="header-icon-btn" id="sidebarProfileIconBtn" title="Profile">
                            <i class='bx bx-user-circle'></i>
                        </button>
                        <div class="sidebar-profile-dropdown" id="sidebarProfileDropdown">
                            <a href="#" class="sidebar-dropdown-item" id="sidebarProfileLink">
                                <i class='bx bx-user'></i>
                                <span>Profile</span>
                            </a>
                            <div class="sidebar-dropdown-divider"></div>
                            <a href="logout.php" class="sidebar-dropdown-item sidebar-dropdown-signout">
            ...
            <div class="sidebar-profile">
                <div class="sidebar-avatar"><?= htmlspecialchars($initials, ENT_QUOTES) ?></div>
                <div class="sidebar-user-name"><?= htmlspecialchars($user_name, ENT_QUOTES) ?></div>
                <div class="sidebar-user-email"><?= htmlspecialchars($user_email, ENT_QUOTES) ?></div>
            </div>
            ...
            <div class="navbar-container">
                <div class="navbar-content">
                    ...
                    <div class="navbar-right">
                        ...
                        <button class="icon-btn" id="fullscreenBtn" title="Toggle Fullscreen"><i class='bx bx-fullscreen'></i></button>
...
fullscreenBtn.addEventListener('click', () => {
    if (!document.fullscreenElement) {
        ...
        localStorage.setItem('fullscreen_pref', 'on');
    } else {
        document.exitFullscreen().catch(() => {});
        localStorage.setItem('fullscreen_pref', 'off');
    }
});

document.addEventListener('fullscreenchange', () => {
    updateFullscreenIcon();
    // If user pressed Escape to exit, clear the preference
    if (!document.fullscreenElement) {
        localStorage.setItem('fullscreen_pref', 'off');
    }
});

// Re-enter fullscreen on page load if preference is on
if (localStorage.getItem('fullscreen_pref') === 'on') {
    document.documentElement.requestFullscreen().catch(() => {});
}
        ...
        const sidebarProfileIconBtn = document.getElementById('sidebarProfileIconBtn');
        const sidebarProfileDropdown = document.getElementById('sidebarProfileDropdown');

        if (sidebarNotifBtn) {
            sidebarNotifBtn.addEventListener('click', () => {
                // Wire to your notifications panel as needed
            });
        }
        if (sidebarProfileIconBtn && sidebarProfileDropdown) {
            sidebarProfileIconBtn.addEventListener('click', e => {
                e.stopPropagation();
                sidebarProfileDropdown.classList.toggle('show');
            });
            document.addEventListener('click', e => {
                if (!sidebarProfileDropdown.contains(e.target) && e.target !== sidebarProfileIconBtn) {
