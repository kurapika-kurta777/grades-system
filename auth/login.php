<?php
// ── No error output to browser in any environment ────────────────────────────
ini_set('display_errors', 0);
ini_set('display_startup_errors', 0);
error_reporting(E_ALL);                // Still log to server log
...
// ── Session expiry notice (set by session.php) ────────────────────────────────
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Brute force check ─────────────────────────────────────────────────────
    if ($rl->isLocked($email, $client_ip)) {
        $remaining = $rl->getRemainingLockout($email, $client_ip);
        $mins = ceil($remaining / 60);
        $error = "Too many failed attempts. Account is locked. Try again in {$mins} minute" . ($mins !== 1 ? 's' : '') . ".";
        logAction($conn, 0, "LOCKOUT HIT: login blocked for email=$email IP=$client_ip");
    } else {

        $stmt = $conn->prepare(
            "SELECT user_id, password_hash, role_id, is_active, full_name FROM users WHERE email = ?"
        );
        ...
        if ($row) {
            if ((int)$row['is_active'] === 0) {
                ...
                // Record as a failure attempt (deactivated accounts still count)
                $rl->recordFailure($email, $client_ip);

            } elseif (password_verify($pass, $row['password_hash'])) {
                // ── Successful login ──────────────────────────────────────────
                $rl->clearFailures($email, $client_ip);
                ...
                // ── Rehash if algorithm/cost changed (PASSWORD_DEFAULT upgrades) ──
                ...
                logAction($conn, (int)$row['user_id'], "User logged in from IP $client_ip");
                ...
                // ── Wrong password ────────────────────────────────────────────
                $rl->recordFailure($email, $client_ip);
                $remaining_attempts = $rl->remainingAttempts($email, $client_ip);
                logAction($conn, 0, "Failed login attempt for email=$email from IP=$client_ip");

                if ($remaining_attempts <= 0) {
                    $lockout_mins = ceil(RATE_LIMIT_LOCKOUT / 60);
                    $error = "Invalid credentials. Account is now locked for {$lockout_mins} minutes due to too many failed attempts.";
                } elseif ($remaining_attempts <= 2) {
                    $error = "Invalid credentials. {$remaining_attempts} attempt" . ($remaining_attempts !== 1 ? 's' : '') . " remaining before lockout.";
                } else {
                    $error = "Invalid credentials. Access denied.";
                }
            }
        } else {
            // ── No such email — record IP failure to prevent enumeration via timing ──
            $rl->recordFailure($email, $client_ip);
            logAction($conn, 0, "Failed login attempt for unknown email=$email from IP=$client_ip");
            // Deliberate: same message as wrong password to prevent user enumeration
            $error = "Invalid credentials. Access denied.";
        }
    }
}
...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .chev-2 {
            position: absolute; top: 0; left: 0; bottom: 0; width: 108%;
        ...
        .right::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--gold), var(--gold-lt), var(--accent)); }
        ...
        .warning-alert i { color: var(--warning);  font-size: 1rem; margin-top: 1px; flex-shrink: 0; }
        .error-alert span   { font-size: 0.8125rem; font-weight: 500; color: #7f1d1d; line-height: 1.5; }
        .warning-alert span { font-size: 0.8125rem; font-weight: 500; color: #78350f; line-height: 1.5; }

        .fgrid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem 1.25rem; }
        .s2 { grid-column: 1 / -1; }

        .fld { display: flex; flex-direction: column; gap: 4px; }
        .fld label { display: flex; align-items: center; gap: 5px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.10em; text-transform: uppercase; color: var(--blue-dark); }
        ...
        .fi {
            ...
            font-weight: 450; color: var(--ink); background: var(--white);
            outline: none; transition: var(--transition); appearance: none;
        }
        .fi::placeholder { color: var(--ink-faint); font-weight: 400; }
        .fi:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(37,99,235,0.10); }
        .fi.err { border-color: #fca5a5; background: #fff8f8; }

        .ii { position: absolute; left: 0.7rem; top: 50%; transform: translateY(-50%); font-size: 1rem; color: var(--blue-light); pointer-events: none; transition: color 0.17s; z-index: 2; }
        .iw:focus-within .ii { color: var(--accent); }

        .peye { position: absolute; right: 0.7rem; top: 50%; transform: translateY(-50%); font-size: 1rem; color: var(--ink-faint); cursor: pointer; z-index: 3; transition: color 0.17s; background: none; border: none; padding: 4px; }
        .peye:hover { color: var(--accent); }

        .sbtn {
            margin-top: 1.1rem; width: 100%; padding: 0.9rem;
        ...
        .security-notice { display: flex; align-items: flex-start; gap: 10px; background: var(--blue-faint); border: 1px solid var(--blue-pale); border-radius: 10px; padding: 0.875rem 1rem; margin-top: 1rem; }
        .security-icon { width: 28px; height: 28px; background: linear-gradient(135deg, var(--accent), var(--blue-dark)); border-radius: 7px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    ...
    <div class="right-inner">
        ...
        <?php if (!empty($warning)): ?>
            <div class="warning-alert">
                ...
                <span><?= htmlspecialchars($warning, ENT_QUOTES) ?></span>
            </div>
        <?php endif; ?>
        ...
        <div class="security-notice">
            ...
            <div class="security-text">
                ...
                This system is for SIEMS-registered users only. All access attempts are logged and monitored. Repeated failed attempts will result in a temporary lockout.
            </div>
        </div>
