<?php
require_once __DIR__ . '/includes/security_headers.php';   // ← NEW (must be first)
...
// ── Redirect if session expired (set by session.php) ─────────
...
if ($page === 'submit_grades' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_components') {
    ...
    // Also detect fetch requests which don't always set X-Requested-With
    ...
    // Read term from POST
    $term = isset($_POST['term']) ? trim($_POST['term']) : 'Prelim';
    if (!in_array($term, ['Prelim', 'Midterm', 'Finals'])) {
        $term = 'Prelim';
    }
    ...
    try {
        // Delete existing categories for this subject and term (cascade deletes items)
        $stmt = $conn->prepare("DELETE FROM grade_categories WHERE subject_id = ? AND term = ?");
        ...
        if (!empty($categories_data)) {
            foreach ($categories_data as $cat_data) {
                ...
                // Insert category with term and input_mode
                $stmt = $conn->prepare("INSERT INTO grade_categories (subject_id, term, category_name, weight, input_mode) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("issds", $subject_id, $term, $name, $weight, $inputMode);
                $stmt->execute();
        ...
        $conn->commit();
        logAction($conn, $faculty_id, "Updated $term grade components for subject $subject_id");

        if ($is_ajax || $is_fetch) {
            header('Content-Type: application/json');
            echo json_encode(['success' => true, 'message' => 'Components saved successfully.']);
            exit;
        }
        ...
        if ($is_ajax || $is_fetch) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Error saving components: ' . $e->getMessage()]);
            exit;
        }
